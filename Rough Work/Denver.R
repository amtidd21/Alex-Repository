...
schedule_DU <- schedule |> filter(away_team == "Denver" | home_team == "Denver")
...
dates_vec <- unique(schedule_DU$date)
...
for (i in dates_vec) {
  new_rankings <- update_rankings(season = schedule_DU, game_date = i, ratings = new_rankings, k = 100)
...
try_1_DU = update_rankings_iter(season = schedule_DU, end_date = "2024-03-23", ratings = rankings, k = 100)
