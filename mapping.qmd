...
k = seq(51, 52, length.out = 10)
home_ice = seq(64, 65, length.out = 10)
d = seq(51, 52, length.out = 10)

grid = expand.grid(k = seq(51, 52, length.out = 10), home_ice = seq(64, 65, length.out = 10), d = seq(51, 52, length.out = 10))

mean_residuals = with_progress({future_pmap_dbl(grid, \ (k, home_ice, d) update_rankings_residuals(season = schedule, end_date = "2025-03-25", ratings = rankings2324, k = k, home_ice = home_ice, d = d), .progress = TRUE)})
...
## BOOM our model is optimized with k = 51, home_ice = 64, d = 51.556, mean_residual = 0.3515844
```

```{r}
apr_5_ranking = update_rankings_iter_gd_ha(schedule, "2025-04-05", rankings2324, 51, 64, 0.3515844)[[108]]
```
